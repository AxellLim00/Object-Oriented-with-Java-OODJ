...
public class Category implements Updateable{
    ...
    public Category(String ID, boolean flag){
        this.item.filename = "src/Database/Category.txt";
        this.item.tempname = "src/Database/TempCategory.txt";
        this.item.id = ID;
    }
    ...
    public boolean create() {
        ...
        try(PrintWriter p = new PrintWriter(new FileWriter(this.item.filename, true))){
            this.item.data = String.join("|", this.item.id, this.item.name, this.description);
        ...
        try(Scanner read = new Scanner(new File(this.item.filename))){
            while (read.hasNextLine()) {
                ...
                if (this.item.name.equals(this.item.record.get(1))||this.item.id.equals(this.item.record.get(0))) {
                    ...
                    this.description = this.item.record.get(2);
    ...
    public String toString() { 
        String print = String.join("\n", this.item.id, this.item.name, this.description);
        return print;
    }
    ...
    public boolean setData(ArrayList<String> inputrecord){
        try{
            ...
            this.description = inputrecord.get(1);
            this.item.line = String.join("|", this.item.id, this.item.name, this.description);
            return true;
